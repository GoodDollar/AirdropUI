...
const EthHttps = process.env.NEXT_PUBLIC_ETH_HTTPS || "https://cloudflare-eth.com",
      EthId = process.env.NEXT_PUBLIC_ETH_ID,
      FuseHttps = process.env.NEXT_PUBLIC_FUSE_HTTPS || "https://rpc.fuse.io";

const getGOODContract = (env,web3) => {
  const addr = contracts[env].GReputation
  return new web3.eth.Contract(GReputationABI.abi, addr)
}
...
export async function getClaimStatus(currentConnection) {
  ...
  for (const network of contractAddressess){
    ...
    const gRepContract = getGOODContract(network,web3);
...
export async function getRecipient(contractInstance, currentConnection) {
  ...
  const gRepContract = contractInstance ? contractInstance 
                        : getGOODContract(currentConnection.chainId == 122 ? "production" : "production-mainnet", web3)
...
export async function claimReputation(proofData, currentConnection, contractInstance) {
  ...
  let proveBalanceABI
  console.log(gRepContract.methods)
  if(gRepContract.methods.proveBalanceOfAtBlockchainLegacy && proofData.isRightNode.length > 0) //fixed old format
  {
    proveBalanceABI = gRepContract.methods.proveBalanceOfAtBlockchainLegacy(
      chainStateId,
      proofData.addr,
      proofData.reputationInWei,
      proofData.hexProof,
      proofData.isRightNode,
      0
    ).encodeABI();
  }
  else if(proofData.proofIndex) { //old buggy format
    proveBalanceABI = gRepContract.methods.proveBalanceOfAtBlockchain(
      chainStateId,
      proofData.addr,
      proofData.reputationInWei,
      proofData.hexProof,
